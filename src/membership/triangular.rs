...
impl MembershipFn for Triangular{
    ...
    fn eval(&self, x: Float) -> Float{
        //out of bounds check
        if x <= self.left  {return 0.0}
        if x >= self.right {return 0.0}

        //calculation within membership function
        if (x - self.center).abs() < 1e-9      {1.0}
        else if x < self.center {slope(x, self.left, self.center,1.0)}
        else                    {slope(x, self.center, self.right, -1.0)}
    }
}
...
impl Triangular{
    ...
    pub fn new(l:Float, c:Float, r:Float) -> crate::error::Result<Self>{
        ...
        return Ok(Triangular{left: l, center:c, right:r});
    }
}
