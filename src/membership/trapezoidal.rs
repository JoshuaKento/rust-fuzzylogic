...
impl MembershipFn for Trapezoidal{
    ...
    fn eval(&self, x: Float) -> Float{
        ...
        //out of bounds check
        if x <= self.left_leg  {return 0.0}
        if x >= self.right_leg {return 0.0}

        //calculation within the membership function
        if (x - self.left_base).abs() < eps  || (x - self.right_base).abs() < eps    {1.0}
        else if x < self.left_base                       {slope(x, self.left_leg, self.left_base, 1.0)}
        else if x > self.left_base && x < self.right_base{1.0}
        else                                             {slope(x, self.right_base, self.right_leg, -1.0)}
    }
}
...
impl Trapezoidal{
    ...
    pub fn new(ll:Float, lb:Float, rb:Float, rl:Float) -> crate::error::Result<Self>{
        ...
        return Ok(Trapezoidal{left_leg: ll, left_base:lb, right_base:rb, right_leg:rl});
    }
}
...
mod tests{
    ...
    fn test_trapezoidal(){
        ...
        assert_eq!(Trapezoidal::new(0.0,0.0,-1.0, 0.0), Err(FuzzyError::BadArity));
