...
fn validate_positive(val: Float) -> Result<(), FuzzyError>{
    if val <= 0.0{ return Err(FuzzyError::OutOfBounds);}
    Ok(())
}
...
impl Gaussian{
    ...
    pub fn new(sd: Float, mean: Float) -> crate::error::Result<Self>{
        ...
        return Ok(Gaussian{sd: sd, mean: mean, neg_two_sigma_sq: (-2.0*sd.powi(2))});
    }
}
...
mod tests{
    ...
    fn test_gaussian(){
        ...
        assert_eq!(membership.clone().unwrap().eval(-1.0),membership.clone().unwrap().eval(1.0));
    }
}